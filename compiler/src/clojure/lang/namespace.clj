(ns clojure.lang.namespace
  (:refer-clojure :only [defn defn- assoc get-in if-let str let = assoc-in atom or reduce fn val key -> not select-keys doseq when-not contains? format apply dissoc get set concat keys update-in prn])
